...
public class Task3 {
  ...
  public static volatile List<Components>
      robotComponents; // list of required components for one robot

  public static CopyOnWriteArrayList<Components> producedComponents = new CopyOnWriteArrayList<>();
  //	public static List<Components> producedComponents = Collections.synchronizedList(new
  // ArrayList<>());
  //	public static List<Components> producedComponents = new ArrayList<>();				// list of 10
...
class Factory implements Runnable {
  ...
  public void run() {
    while (!Task3.getVictory()) { // stop factory when victory is achieved
      ...
      System.out.println(
          "Factory - day " + counter + ", starting stock: " + Task3.printProducedComponents());
      if (Task3.sizeProducedComponents() < 10) {
        ...
        System.out.println(
            "Factory - day "
                + counter
                + ", stock a the end of a day: "
                + Task3.printProducedComponents());
      }
    }
  }
}
...
class World implements Runnable {
  ...
  @Override
  public void run() {
    while ((Task3.getReady() && (robotsQProduced < Task3.robotsQ))
        && !Task3.getVictory()) { // can collect items
      // when factory is
      // open
      ...
      if (Task3.sizeProducedComponents() > 0) {
        for (Components part :
            Task3.robotComponents) { // Traveling over all required robot components
          //			System.out.println("Element " + part);
          int index =
              Task3.indexProducedComponents(
                  part); // looking for required component in factory store
          if (index > -1) {
            //				System.out.println("Found: " + Task3.producedComponents.indexOf(part));
            producingRobots.add(part); // adding component for robot set
            Task3.removeProducedComponents(
                index); // deleting component from factory store via synchronized
            // method
          }
        }
      }
...
class Wednesday implements Runnable {
  ...
  public void run() {
    while ((Task3.getReady() && (robotsQProduced < Task3.robotsQ))
        && !Task3.getVictory()) { // can collect items when factory is open
